...
import { TrendingUp, BarChart as ChartIcon, Package, Zap, AlertCircle } from 'lucide-react-native';
    ...
    return (<TransitionView index={index} type="scale" style={styles.kpiCard}>
            ...
            <GlassCard style={styles.kpiCardInner}>
                <View style={styles.kpiHeader}>
                    ...
                    {trend && (<View style={[styles.kpiTrendBadge, { backgroundColor: theme.colors.primaryContainer + '40' }]}>
...
export default function AdminAnalytics() {
    ...
    return (<View style={styles.container}>
            ...
            <ScrollView style={styles.content} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} tintColor={theme.colors.primary}/>}>
                <View style={styles.mainContent}>
                    {/* KPI Section */}
                    <View style={styles.kpiRow}>
                        <KPICard index={0} icon={Package} label="Active orders" value={analytics?.stats?.activeOrders?.toString() || "0"} color="#2196F3" trend={`${analytics?.stats?.activeOrdersTrend > 0 ? '+' : ''}${analytics?.stats?.activeOrdersTrend || 0}%`} footer={`${analytics?.stats?.completedToday || 0} completed today`}/>
                        <KPICard index={1} icon={Zap} label="Efficiency rate" value={`${analytics?.stats?.efficiency || 0}%`} color="#4CAF50" trend="+0.0%" footer={`Target: ${analytics?.stats?.targetEfficiency || 90}%`}/>
                        <KPICard index={2} icon={TrendingUp} label="Units produced" value={analytics?.stats?.totalProduced?.toLocaleString() || "0"} color="#FF9800" trend={`${analytics?.stats?.productionTrend > 0 ? '+' : ''}${analytics?.stats?.productionTrend || 0}%`} footer={`vs ${analytics?.stats?.lastWeekProduced || 0} last week`}/>
                        <KPICard index={3} icon={AlertCircle} label="Critical alerts" value={analytics?.stats?.alerts?.toString() || "0"} color="#F44336" trend={analytics?.stats?.alerts > 0 ? `${analytics?.stats?.alerts} items` : null} footer={`${analytics?.stats?.lowStockCount || 0} low-stock warnings`}/>
                    </View>

                    {/* Analytics Section */}
                    <TransitionView index={4}>
                        <Text variant="titleMedium" style={styles.sectionTitle}>Production Analytics</Text>
                        {analytics && analytics.weeklyProduction && analytics.weeklyProduction.length > 0 ? (<View style={styles.analyticsWrapper}>
                                <GlassCard style={styles.chartCard}>
                                    ...
                                    <LineChart data={{
                labels: analytics.weeklyProduction.map((d) => new Date(d.date).toLocaleDateString('en-GB', { day: '2-digit', month: '2-digit' })),
                datasets: [{ data: analytics.weeklyProduction.map((d) => d.total) }]
            }} width={chartWidth} height={180} chartConfig={chartConfig} bezier style={styles.chart}/>
                                </GlassCard>
                            </View>) : (<GlassCard style={styles.analyticsWrapper}>
                    ...
                    <TransitionView index={5}>
                        ...
                        {predictions && predictions.length > 0 && predictions.some(p => typeof p.days_remaining === 'number') ? (<View style={styles.analyticsWrapper}>
                                <GlassCard style={styles.chartCard}>
                                    <View style={styles.chartHeader}>
            ...
            }} width={chartWidth} height={180} yAxisLabel="" yAxisSuffix="d" chartConfig={{ ...chartConfig, color: (opacity = 1) => theme.colors.primary }} style={styles.chart} fromZero/>
                                </GlassCard>
                            </View>) : (<GlassCard style={styles.analyticsWrapper}>
