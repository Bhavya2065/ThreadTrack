...
if (Platform.OS === 'web' && typeof window !== 'undefined') {
    ...
    const createSilencer = (originalFunc) => (...args) => {
        const logString = args.map(a => String(a)).join(' ');
        if (bugPhrases.some(phrase => logString.includes(phrase)))
            return;
      ...
      <ToastProvider>
        <PaperProvider theme={theme}>
          <View style={[styles.root, { backgroundColor: theme.colors.background }]}>
            {Platform.OS === 'web' && (<style dangerouslySetInnerHTML={{ __html: `
                ::-webkit-scrollbar {
                  width: 0px;
              ...
              ` }}/>)}
            {isHideLayout ? (<Stack screenOptions={{ headerShown: false }}/>) : (<>
