...
export const reportExporter = {
    async exportInventoryToPDF(materials, title) {
        const html = `
            <html>
                <head>
                    <style>
    ...
    async exportOrdersToPDF(orders, title) {
        const html = `
            <html>
                <head>
                    <style>
    ...
    async exportInventoryToCSV(materials) {
        const header = 'Material ID,Name,Current Stock,Unit,Min Required,Last Updated\n';
        const rows = materials.map(m => `${m.MaterialID},"${m.Name}",${m.CurrentStock},${m.Unit},${m.MinimumRequired},"${new Date(m.LastUpdated).toLocaleString()}"`).join('\n');
    ...
    async exportOrdersToCSV(orders) {
        ...
        const rows = orders.map(o => `${o.OrderID},"${o.ProductName}","${o.BuyerName}",${o.Quantity},"${o.Status}","${new Date(o.OrderDate).toLocaleString()}","${(o.CompletionNotes || '').replace(/"/g, '""')}"`).join('\n');
        const csvContent = header + rows;
