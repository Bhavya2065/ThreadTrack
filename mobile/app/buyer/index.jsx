...
import { Text, Appbar, TextInput, Button, ProgressBar, useTheme, Portal, Modal } from 'react-native-paper';
import { useRouter } from 'expo-router';
import { Truck, Timer, CheckCircle2, AlertTriangle, ShoppingBag } from 'lucide-react-native';
...
export default function BuyerTracking() {
    ...
    const inquiryInfo = React.useMemo(() => {
        ...
        for (const item of itemsToOrder) {
            ...
            if (item.quantity > maxUnits)
                return { needed: true, amount: maxUnits };
        }
        ...
        return (<View style={styles.container}>
                ...
                <ScrollView style={styles.content}>
                    <View style={styles.mainContent}>
                        ...
                        {[1, 2].map(i => (<SkeletonLoader key={i} height={180} width="100%" style={{ marginBottom: 16 }}/>))}
                    </View>
                </ScrollView>
            </View>);
    }
    if (error) {
        return (<View style={[styles.container, styles.errorContainer]}>
    ...
    return (<View style={styles.container}>
            ...
            <ScrollView style={styles.content} contentContainerStyle={orders.length === 0 ? { flexGrow: 1, justifyContent: 'center' } : null} refreshControl={<RefreshControl refreshing={submitting} onRefresh={() => { Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light); fetchData(); }} tintColor={theme.colors.primary}/>}>
                <View style={styles.mainContent}>
                    {orders.length > 0 && (<View style={styles.headerRow}>
                            <Text variant="titleLarge" style={styles.sectionTitle}>Active Shipments</Text>
                        </View>)}

                    {orders.map((order, index) => (<TransitionView key={order.OrderID} index={index}>
                            <GlassCard style={styles.card}>
                                ...
                                <View style={styles.progressContainer}>
                                    ...
                                    <ProgressBar progress={Math.min(order.ProducedQuantity / order.Quantity, 1)} color={theme.colors.primary} style={styles.progressBar}/>
                                </View>

                                {order.Status !== 'Completed' && order.Status !== 'Cancelled' && (<View style={styles.estDelivery}>
                    ...
                    return new Date(Date.now() + remainingMs).toLocaleDateString('en-GB', { day: '2-digit', month: 'short' });
                                ...
                                {(order.Status !== 'Completed' && order.Status !== 'Cancelled') && (<View style={{ marginTop: 15, flexDirection: 'row', justifyContent: 'flex-end' }}>
                                        <Button mode="outlined" compact icon="close-circle" textColor={theme.colors.error} style={{ borderColor: theme.colors.error }} onPress={() => {
                    ...
                    setIsCancelModalVisible(true);
                                ...
                                {(order.CompletionNotes) && (<View style={[styles.notesContainer, order.Status === 'Cancelled' && { backgroundColor: 'rgba(255, 59, 48, 0.05)', borderColor: 'rgba(255, 59, 48, 0.1)' }]}>
                    ...
                    {orders.length === 0 && (<EmptyState icon={ShoppingBag} title="No Orders Yet" message="Create your first bulk order to start tracking." iconColor={Tokens.colors.primary}/>)}
                </View>
            </ScrollView>

            <Portal>
                <Modal visible={isModalVisible} onDismiss={() => setIsModalVisible(false)} contentContainerStyle={[styles.modalContent, styles.responsiveModal]}>
            ...
            return (<View key={p.ProductID} style={{ marginBottom: Tokens.spacing.md, padding: Tokens.spacing.md, borderRadius: Tokens.borderRadius.md, backgroundColor: 'rgba(255, 255, 255, 0.02)', borderWidth: 1, borderColor: theme.colors.outline }}>
                                        <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                                            <View style={{ flex: 1 }}>
                                                <Text style={styles.productNameText}>{p.ProductName}</Text>
                                            </View>
                                            <TextInput value={orderQuantities[p.ProductID] || ''} onChangeText={(val) => handleQuantityChange(p.ProductID, val)} keyboardType="numeric" mode="outlined" dense style={{ width: 80, height: 40, backgroundColor: 'transparent' }} outlineColor={theme.colors.outline} activeOutlineColor={theme.colors.primary} textColor={theme.colors.onSurface}/>
                                        </View>
                                    </View>);
                    ...
                    {inquiryInfo.needed && (<View style={{ marginTop: 10, padding: 10, backgroundColor: 'rgba(0, 150, 255, 0.1)', borderRadius: 8, flexDirection: 'row', alignItems: 'center', gap: 10 }}>
                    ...
                    <View style={styles.modalButtons}>
                        ...
                        <Button mode="contained" onPress={handleCreateOrder} loading={submitting} disabled={submitting} labelStyle={{ fontWeight: 'normal' }}>
                            {inquiryInfo.needed ? 'Send Bulk Inquiry' : 'Place Order'}
                        </Button>
                    </View>
                </Modal>

                <Modal visible={isCancelModalVisible} onDismiss={() => {
            setIsCancelModalVisible(false);
            setCancelReason('');
            setSelectedOrderId(null);
                    ...
                    <TextInput label="Cancellation Reason" value={cancelReason} onChangeText={setCancelReason} mode="outlined" multiline numberOfLines={3} style={{ backgroundColor: 'transparent', marginBottom: 20 }} placeholder="e.g., Change of plans, Ordered by mistake..."/>
            ...
            setSelectedOrderId(null);
                        ...
                        <Button mode="contained" onPress={handleCancelOrder} loading={submitting} disabled={submitting || !cancelReason.trim()} buttonColor={theme.colors.error}>
                            Confirm Cancel
                        </Button>
                    </View>
                </Modal>

                <Modal visible={isConfirmCancelModalVisible} onDismiss={() => setIsConfirmCancelModalVisible(false)} contentContainerStyle={[styles.modalContent, styles.responsiveModal]}>
                    ...
                    <View style={styles.modalButtons}>
                        <Button onPress={() => setIsConfirmCancelModalVisible(false)} disabled={submitting} textColor={theme.colors.onSurface}>
                            No, Go Back
                        </Button>
                        <Button mode="contained" onPress={finalConfirmCancel} loading={submitting} disabled={submitting} buttonColor={theme.colors.error} labelStyle={{ fontWeight: 'bold' }}>
                            Yes, Cancel it
                        </Button>
                    </View>
                </Modal>
