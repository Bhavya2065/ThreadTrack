...
const TOAST_CONFIG = {
    success: {
        accentColor: '#22C55E',
        iconBg: '#22C55E',
        icon: (size) => <CheckCircle2 size={size} color="#FFFFFF"/>,
    },
    error: {
        accentColor: '#EF4444',
        iconBg: '#EF4444',
        icon: (size) => <AlertCircle size={size} color="#FFFFFF"/>,
    },
    warning: {
        accentColor: '#F59E0B',
        iconBg: '#F59E0B',
        icon: (size) => <AlertTriangle size={size} color="#FFFFFF"/>,
    },
    info: {
        accentColor: '#3B82F6',
        iconBg: '#3B82F6',
        icon: (size) => <Info size={size} color="#FFFFFF"/>,
    },
};
export const ToastProvider = ({ children }) => {
    const [visible, setVisible] = useState(false);
    const [options, setOptions] = useState({ title: '', message: '', type: 'info' });
    ...
    onMouseLeave={Platform.OS === 'web' ? handleMouseLeave : undefined} style={[
            styles.container,
            {
                opacity: fadeAnim,
