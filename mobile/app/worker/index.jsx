...
import { Text, TextInput, Button, Appbar, List, Divider, useTheme } from 'react-native-paper';
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            if (activeOrders.length > 0) {
                const currentStillActive = activeOrders.some((o) => o.OrderID === selectedOrderId);
                        ...
                        <GlassCard>
                            {orders.length > 0 ? (<>
                                    ...
                                    <View style={styles.orderSelection}>
                                        {offlineCount > 0 && (<View style={{ backgroundColor: 'rgba(255, 171, 0, 0.1)', padding: 12, borderRadius: 8, marginBottom: 16, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', borderTopWidth: 1, borderBottomWidth: 1, borderColor: 'rgba(255, 171, 0, 0.2)' }}>
                                        ...
                                        {orders.map((order, index) => (<Button key={`order-${order.OrderID}-${index}`} mode={selectedOrderId === order.OrderID ? "contained" : "outlined"} onPress={() => {
                    Haptics.selectionAsync();
                    setSelectedOrderId(order.OrderID);
                }} style={[styles.orderChip, selectedOrderId === order.OrderID && { borderColor: theme.colors.primary, borderWidth: 1.5 }]} labelStyle={{ fontSize: 11, fontWeight: '700' }} textColor={theme.colors.onSurface} buttonColor={selectedOrderId === order.OrderID ? 'rgba(0, 97, 255, 0.1)' : 'transparent'}>
                                                #{order.OrderID}
                                            </Button>))}
                                    </View>

                                    {selectedOrderId && (<View style={styles.progressSection}>
                                            {(() => {
                                ...
                                ...Platform.select({
                                    ...
                                    default: {
                                        ...
                                        shadowRadius: 10,
                                                        ...
                                                        {remaining === 0 && (<Text style={styles.completedHint}>Ready for review</Text>)}
                                                    </>);
                })()}
                                        </View>)}

                                    <TextInput label="Product" value={orders.find(o => o.OrderID === selectedOrderId)?.ProductName || 'Select an order...'} mode="outlined" disabled style={styles.input} outlineColor={theme.colors.outline} activeOutlineColor={theme.colors.primary} textColor={theme.colors.onSurface}/>

                                    <TextInput label="Units Completed" value={quantity} onChangeText={setQuantity} mode="outlined" keyboardType="numeric" style={styles.input} placeholder="0" outlineColor={theme.colors.outline} activeOutlineColor={theme.colors.primary} textColor={theme.colors.onSurface}/>

                                    <Button mode="contained" onPress={() => { Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Medium); handleSubmit(); }} style={styles.submitButton} icon="check" loading={submitting} disabled={submitting || (orders.find(o => o.OrderID === selectedOrderId)?.Quantity - orders.find(o => o.OrderID === selectedOrderId)?.ProducedQuantity <= 0)} labelStyle={{ fontWeight: '900' }}>
                                        Log Production
                                    </Button>
                                </>) : (<EmptyState icon={logs.length === 0 ? PackageOpen : CheckCircle} title={logs.length === 0 ? "No Orders Available" : "All Orders Completed"} message={logs.length === 0
                ? "There are currently no orders in manufacturing. New orders will appear here once assigned."
                : "Great job! You have finished all active orders. New orders will appear here once assigned."} iconColor={logs.length === 0 ? theme.colors.outline : Tokens.colors.success}/>)}
                        </GlassCard>
                    </TransitionView>
                    ...
                    <TransitionView index={1}>
                        ...
                        <GlassCard style={{ padding: 0 }}>
                            {logs.map((log, index) => (<View key={`log-${log.LogID || index}-${index}`}>
                                    <List.Item title={`${log.QuantityProduced} ${log.ProductName}`} titleStyle={{ color: theme.colors.onSurface, fontWeight: '600' }} description={new Date(log.LogDate).toLocaleString('en-GB', { day: '2-digit', month: '2-digit', hour: '2-digit', minute: '2-digit' })} descriptionStyle={{ color: theme.colors.onSurfaceVariant, fontSize: 11 }} left={props => <CheckCircle {...props} color={theme.colors.primary} size={20} style={styles.logIcon}/>}/>
                                    {index < logs.length - 1 && <Divider style={{ backgroundColor: theme.colors.outline, opacity: 0.3 }}/>}
                                </View>))}
                            {logs.length === 0 && (<EmptyState icon={History} title="No Recent Activity" message="Logs will appear here once submitted."/>)}
                        </GlassCard>
                    </TransitionView>
                </View>
            </ScrollView>
